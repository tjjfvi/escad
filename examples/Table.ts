...
const beam = Component.create(
  "beam",
  (
    orientation: BeamOrientation,
    length: number,
    xyz: XYZ,
  ): ConvertibleElement<Mesh> => {
    let [x, y, z] = "xyz".split("").map((n) =>
      orientation[0] === n ? length : orientation[1] === n ? 3.5 : 1.5
    );
    return escad.cube({ x, y, z }).translate(...xyz);
  },
);

const bracket = Component.create(
  "bracket",
  (orientation: BracketOrientation, xyz: XYZ): ConvertibleElement<Mesh> => {
