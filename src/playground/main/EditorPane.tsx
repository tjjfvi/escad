...
`;
  ...
  mainModel.onDidChangeContent(debounce(async () => {
    ...
    history.replaceState(
      {},
      "Playground",
      "#code=" + lzstring.compressToEncodedURIComponent(content),
    );
  ...
  function syncChangedFiles() {
    ...
    for (const changed of changedFiles) {
      if (changed === "/project/index.ts" || !changed.startsWith("/project/")) {
        continue;
      }
  ...
  function getTypesField(file: string, content: string) {
    ...
    return path.resolve(
      path.dirname(file),
      json.types ?? json.typings ?? "index.d.ts",
    );
  }

  function getModuleFileName(file: string) {
    const partialPath =
      file.split("node_modules/").slice(-1)[0].split("@types/").slice(-1)[0];
  ...
  function getModuleName(file: string) {
    ...
    return moduleFileName.split("/").slice(0, moduleFileName[0] === "@" ? 2 : 1)
      .join("/");
  }

  function syncDeps(file: string, content: string) {
    const regex =
      /((?:from|import)\s+(["']))(.+?)\2|((?:import|require)\s*\(\s*(["']))(.+?)(\5\s*\))/g;
