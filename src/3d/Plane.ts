...
export const Plane = {
  ...
  flip: (plane: Plane): Plane =>
    Plane.create(Vector3.negate(plane.normal), -plane.w),
  splitFace(
    plane: Plane,
    face: Face,
    { front, back, coplanarFront, coplanarBack }: SplitFaceArguments,
  ) {
    const Coplanar = 0;
    const Front = 1;
    const Back = 2;
