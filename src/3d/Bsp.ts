...
export const Bsp = {
  ...
  mapExtra: <E>(
    bsp: [Bsp, E],
    fn: (bsp: Bsp | null, extra: E) => [PartialBsp | null, [E, E] | null],
  ) => {
    ...
    for (const [origNode, origExtra] of nodeStack) {
      ...
      let node = mappedNode;
      while (
        node === null || node.front !== undefined && node.back !== undefined
      ) {
  ...
  clipFaces: arrayify(
    function* (bsp: Bsp, faces: readonly Face[], options: ClipOptions) {
        ...
        if (node.plane) {
          ...
          for (const face of faces) {
            Plane.splitFace(node.plane, face, {
              ...
              back: backFaces,
              coplanarFront:
                !!(options & ClipOptions.DropCoplanarFront) === dropFront
                  ? frontFaces
                  : backFaces,
              coplanarBack:
                !!(options & ClipOptions.DropCoplanarBack) === dropFront
                  ? frontFaces
                  : backFaces,
            });
          }
