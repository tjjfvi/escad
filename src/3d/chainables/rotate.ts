...
> = Component.create("rotate", (...args: RotateArgs) => {
  ...
  let m = Matrix4.multiply(
    Matrix4.multiply(Matrix4.rotateX(x), Matrix4.rotateY(y)),
    Matrix4.rotateZ(z),
  );

  return mapOperation(
    "rotate",
    (leaf) => Transform.create(TupleProduct.create([m, leaf] as const)),
    { showOutput: false },
  );
}, { showOutput: false });

export const rX: Component<
  [number],
  ConvertibleOperation<Mesh, Transformation<Mesh>>
> = Component.create("rX", (n: number) => rotate({ x: n }), {
  showOutput: false,
});
export const rY: Component<
  [number],
  ConvertibleOperation<Mesh, Transformation<Mesh>>
> = Component.create("rY", (n: number) => rotate({ y: n }), {
  showOutput: false,
});
export const rZ: Component<
  [number],
  ConvertibleOperation<Mesh, Transformation<Mesh>>
