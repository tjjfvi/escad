...
export const createServerClientMessenger = ({
  ...
  async function getRenderer() {
    ...
    do renderer = await (lastRendererProm = currentRendererProm); while (
      lastRendererProm !== currentRendererProm
    ); // Some time has passed, so there may be a new renderer
    return renderer;
  }
  ...
  function reloadRenderer() {
    ...
    currentRendererProm = createRendererMessenger(lookupRaw).then(
      (renderer) => {
