...
export const serializeConnection = (
  _connection: Connection<Uint8Array, unknown>,
): Connection<unknown> => {
  const connection: Connection<Uint8Array> = transformConnection(
    _connection,
    (x) => x,
    (v) => {
  ...
  ee.emit(
    "recv",
    wrap($unknown.deserializeStreamAsync(unwrap(ee.on("_recv")))),
  );
  return {
...
async function* unwrap<T>(
  iterable: AsyncIterable<readonly [T]>,
): AsyncIterable<T> {
