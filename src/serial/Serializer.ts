...
export class Serializer<T> {
  ...
  constructor(
    { s, d }: { s: (value: T) => Serialization; d: () => Deserialization<T> },
  ) {
    this.s = s;
    this.d = d;
  }
  ...
  static *write(
    length: number,
    fn: (buffer: BufferInfo) => void,
  ): Serialization {
    const context = Serializer.serializationContext;
    if (!context) {
      throw new Error(
        "Serializer.write iterator cannot be called outside of Serializer#serialize",
      );
    }
    ...
    context.offset = length;
    context.chunk = BufferInfo.create(
      new Uint8Array(Math.max(context.chunkSize, length)),
      0,
    );
    fn(context.chunk);
  }
  ...
  static *read<T>(
    length: number,
    fn: (buffer: BufferInfo) => T,
  ): Deserialization<T> {
    const context = Serializer.deserializationContext;
    if (!context) {
      throw new Error(
        "Serializer.read iterator cannot be called outside of Serializer#deserialize",
      );
    }
  ...
  *serialize(
    value: T,
    { chunkSize = 2 ** 16 }: SerializeOptions = {},
  ...
  *serializeStream(iterable: Iterable<T>, options: SerializeOptions = {}) {
  ...
  private *_deserializeStream(): Generator<
    undefined | { value: T },
    void,
  ...
  *deserializeStream(stream: Iterable<Uint8Array>): Iterable<T> {
  ...
  private static _finishSerializeChunk(context: SerializationContext) {
    return new Uint8Array(
      context.chunk.buffer.buffer,
      context.chunk.buffer.byteOffset,
      context.offset,
    );
  }
}
