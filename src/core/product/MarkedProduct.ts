...
export const MarkedProduct = {
  ...
  getMarkedProductType: <T extends MarkedProduct>(
    product: T,
  ): MarkedProductType<T> =>
    MarkedProductType.create(
      product.marker,
      Product.getProductType(product.child),
    ) as never,
  for: <T extends Marker>(marker: T) => ({
    create: <U extends Product>(child: U) =>
      MarkedProduct.create(marker, child),
    createProductType: <U extends Product>(child: ProductTypeish<U>) =>
      MarkedProductType.create(marker, child),
  }),
};
