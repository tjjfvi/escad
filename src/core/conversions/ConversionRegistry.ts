...
export class ConversionRegistry {
  ...
  readonly excludeStores: ReadonlySet<ArtifactStore> = new Set([
    this.artifactStore,
  ]);
  ...
  private async finishPath(
    initialFromType: ProductType,
    path: ConversionPath | null,
    finalToType: ProductType,
  ) {
    if (!path) {
      return null;
    }
  ...
  private maybeImplicitlyConvertibleTo(
    fromType: ProductType,
    toType: ProductType,
  ): boolean {
    return false ||
  ...
  private async finishPathSegment(
    ...
    if (HashProductType.isHashProductType(fromType)) {
      const resolvedFromType = await this.artifactManager.lookupRaw(
        fromType.productType,
      );
      if (!resolvedFromType) {
    ...
    if (HashProductType.isHashProductType(toType)) {
      const resolvedToType = await this.artifactManager.lookupRaw(
        toType.productType,
      );
    ...
    if (UnknownProductType.isUnknownProductType(toType)) {
      return {
        fromType,
        toType,
        convert: async (product: Product) => UnknownProduct.create(product),
        weight: 0,
      };
    }
    ...
    if (
      ...
      return subPath && {
        ...
        convert: async (product: MarkedProduct) =>
          MarkedProduct.create(
            product.marker,
            await this.executeConversionPath(product.child, subPath),
          ),
        weight: this.weight(subPath),
      };
    }
    ...
    if (
      ...
      return {
        fromType: TupleProductType.create(
          Array(fromType.elementTypes.length).fill(toType.elementType),
        ),
        toType,
    ...
    if (
      ...
      const subPath = await this.compose(
      ...
      return subPath && {
        ...
        convert: async (product: ArrayProduct) =>
          ArrayProduct.create(
            await Promise.all(product.children.map((child) =>
    ...
    if (
      ...
      const subPaths = await Promise.all(
        fromType.elementTypes.map((subFromType, i) =>
      ...
      return {
        ...
        convert: async (product: TupleProduct<any[]>) =>
          TupleProduct.create(
            await Promise.all(product.children.map((child, i) =>
              this.executeConversionPath(child, subPaths[i]!)
            )),
          ),
        weight: subPaths.map((subPath) => this.weight(subPath!)).reduce((
          a,
          b,
        ) => a + b),
      };
  ...
  private async executeConversionPath(
    product: Product,
  ...
  async convertProduct<T extends Product, F extends ConvertibleTo<T>>(
    ...
    if (!conversions) {
      throw new Error(
        `Could not find path to convert product type ${
          Hash.create(fromType)
        } to ${Hash.create(toType)}`,
      );
    }
  ...
  private rehydrateConversionPath(
    path: Array<Omit<ConversionImpl<any, any>, "convert">>,
  ): ConversionPath {
    ...
    const rehydrated: ConversionImpl<any, any> = {
      ...
      convert: async (product: Product): Promise<Product> => {
        ...
        if (id) {
          ...
          const computed = await this.finishPathSegment(
            rehydrated.fromType,
            rehydrated.toType,
          );
          if (
            !computed || !Hash.equal(fromType, computed.fromType) ||
            !Hash.equal(toType, computed.toType)
          ) {
            throw new Error(
              `Could not create conversion from ${Hash.create(fromType)} to ${
                Hash.create(toType)
              }`,
            );
          }
